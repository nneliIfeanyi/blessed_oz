...
if (isset($_POST['itemDetailsItemNumber'])) {
	...
	if (!empty($itemNumber)) {
		...
		$itemSql = 'SELECT itemNumber FROM item WHERE itemNumber=:itemNumber AND storeID = :storeID';
		$itemStatement = $conn->prepare($itemSql);
		$itemStatement->execute(['itemNumber' => $itemNumber, 'storeID' => $activeStoreID]);

		if ($itemStatement->rowCount() > 0) {

			// Item exists in DB. Hence start the DELETE process
			$deleteItemSql = 'DELETE FROM item WHERE itemNumber=:itemNumber AND storeID = :storeID';
			$deleteItemStatement = $conn->prepare($deleteItemSql);
			$deleteItemStatement->execute(['itemNumber' => $itemNumber, 'storeID' => $activeStoreID]);
