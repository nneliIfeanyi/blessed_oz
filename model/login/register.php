...
if (isset($_POST['registerUsername'])) {
	$totalUsers = (int) $conn->query('SELECT COUNT(*) FROM user')->fetchColumn();
	$bootstrapMode = $totalUsers === 0;
	if (!$bootstrapMode && !userCanManageUsers()) {
		echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Only super admin can register new users.</div>';
		exit();
	}
	...
	if (!empty($registerFullName) && !empty($registerUsername) && !empty($registerPassword1) && !empty($registerPassword2)) {
		...
		if ($usernameCheckingStatement->rowCount() > 0) {
			...
			if ($registerPassword1 !== $registerPassword2) {
				...
				$newUserRole = $bootstrapMode ? 'super_admin' : 'admin';
				$insertUserSql = 'INSERT INTO user(fullName, username, password, role, status) VALUES(:fullName, :username, :password, :role, :status)';
				$insertUserStatement = $conn->prepare($insertUserSql);
				$insertUserStatement->execute(['fullName' => $registerFullName, 'username' => $registerUsername, 'password' => $hashedPassword, 'role' => $newUserRole, 'status' => 'Active']);

				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Registration complete. Role assigned: ' . htmlspecialchars($newUserRole) . '. Redirecting to settings...</div><!--REDIRECT:settings.php-->';
				exit();
			}
		}
	} else {
