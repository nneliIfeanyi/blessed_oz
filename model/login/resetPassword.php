	...
	if (!empty($resetPasswordUsername) && !empty($resetPasswordPassword1) && !empty($resetPasswordPassword2)) {
		...
		if ($usernameCheckingStatement->rowCount() < 1) {
			...
			if ($resetPasswordPassword1 !== $resetPasswordPassword2) {
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Password reset complete. Redirecting to settings...</div><!--REDIRECT:settings.php-->';
				exit();
			}
		}
	} else {
