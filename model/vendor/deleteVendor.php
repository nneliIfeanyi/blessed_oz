...
if (isset($_POST['vendorDetailsVendorID'])) {
	...
	if (!empty($vendorDetailsVendorID)) {
		...
		$vendorSql = 'SELECT vendorID FROM vendor WHERE vendorID=:vendorID AND storeID = :storeID';
		$vendorStatement = $conn->prepare($vendorSql);
		$vendorStatement->execute(['vendorID' => $vendorDetailsVendorID, 'storeID' => $activeStoreID]);

		if ($vendorStatement->rowCount() > 0) {

			// Vendor exists in DB. Hence start the DELETE process
			$deleteVendorSql = 'DELETE FROM vendor WHERE vendorID=:vendorID AND storeID = :storeID';
			$deleteVendorStatement = $conn->prepare($deleteVendorSql);
			$deleteVendorStatement->execute(['vendorID' => $vendorDetailsVendorID, 'storeID' => $activeStoreID]);
