...
if (isset($_POST['vendorDetailsVendorID'])) {
	...
	if (!empty($vendorDetailsVendorID)) {
		...
		if (!empty($vendorDetailsVendorFullName) && !empty($vendorDetailsVendorMobile) && !empty($vendorDetailsVendorAddress)) {
			...
			$vendorIDSelectSql = 'SELECT vendorID FROM vendor WHERE vendorID = :vendorID AND storeID = :storeID';
			$vendorIDSelectStatement = $conn->prepare($vendorIDSelectSql);
			$vendorIDSelectStatement->execute(['vendorID' => $vendorDetailsVendorID, 'storeID' => $activeStoreID]);
			...
			if ($vendorIDSelectStatement->rowCount() > 0) {
				...
				$purchaseVendorNameSql = 'UPDATE purchase SET vendorName = :vendorName WHERE vendorID = :vendorID AND storeID = :storeID';
				$purchaseVendorNameStatement = $conn->prepare($purchaseVendorNameSql);
				$purchaseVendorNameStatement->execute(['vendorName' => $vendorDetailsVendorFullName, 'vendorID' => $vendorDetailsVendorID, 'storeID' => $activeStoreID]);

				// Construct the UPDATE query
				$updateVendorDetailsSql = 'UPDATE vendor SET fullName = :fullName, email = :email, mobile = :mobile, phone2 = :phone2, address = :address, address2 = :address2, city = :city, district = :district, status = :status WHERE vendorID = :vendorID AND storeID = :storeID';
				$updateVendorDetailsStatement = $conn->prepare($updateVendorDetailsSql);
				$updateVendorDetailsStatement->execute(['fullName' => $vendorDetailsVendorFullName, 'email' => $vendorDetailsVendorEmail, 'mobile' => $vendorDetailsVendorMobile, 'phone2' => $vendorDetailsVendorPhone2, 'address' => $vendorDetailsVendorAddress, 'address2' => $vendorDetailsVendorAddress2, 'city' => $vendorDetailsVendorCity, 'district' => $vendorDetailsVendorDistrict, 'vendorID' => $vendorDetailsVendorID, 'status' => $vendorDetailsStatus, 'storeID' => $activeStoreID]);

				// UPDATE vendor name in purchase table too
				$updateVendorInPurchaseTableSql = 'UPDATE purchase SET vendorName = :vendorName WHERE vendorID = :vendorID AND storeID = :storeID';
				$updateVendorInPurchaseTableStatement = $conn->prepare($updateVendorInPurchaseTableSql);
				$updateVendorInPurchaseTableStatement->execute(['vendorName' => $vendorDetailsVendorFullName, 'vendorID' => $vendorDetailsVendorID, 'storeID' => $activeStoreID]);
